...
class Silo extends CI_Controller
{
    ...
    public function index()
    {
        if ($this->input->is_ajax_request()) {
            ...
            if ($response['find']) {
                ...
                {
                    $options = [
                        'edit' => '<a title="Editar Registro" href="' . base_url('materia_prima/cadastro/silo/frm/' . $response['find'][$i]['id']) . '" class="btn btn-sm btn-dark"><i class="fas fa-pencil-alt fa-fw"></i> Editar</a>',
        ...
        $this->load->view('raw_material/silo/list', $data);
    ...
    public function form($id = null)
    {
        ...
        if ($id && !$this->silo_model->find_by_id($id)) {
            $this->session->set_flashdata('notification', CONF_MESSAGE_UPDATE_INVALID);

            redirect('materia_prima/cadastro/silo/lst');
        }

        if ($id) {
            $silo = $this->silo_model->find_by_id($id);
        } else {
            $silo = [
                'id' => '',
                'description' => ''
        ...
        $this->load->view('template/header_html');
        $this->load->view('template/header', $header);
        $this->load->view('raw_material/silo/form', $data);
        $this->load->view('template/footer');
        $this->load->view('template/footer_html');
    }
    ...
    {
        if ($this->silo_model->save($id)) {
            $this->session->set_flashdata('notification', CONF_MESSAGE_UPDATE_SUCCESS);

            $response = [
                'redirect' => base_url('materia_prima/cadastro/silo/lst')
            ];
    ...
    public function delete()
    {
        if ($this->input->is_ajax_request()) {
            ...
            if (!$this->silo_model->find_by_id($this->input->post('id'))) {
                $response = [
            ...
            if ($this->silo_model->destroy($this->input->post('id'))) {
