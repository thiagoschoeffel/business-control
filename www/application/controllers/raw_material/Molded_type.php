...
class Molded_type extends CI_Controller
{
    ...
    public function index()
    {
        if ($this->input->is_ajax_request()) {
            $response = [
                'find' => $this->molded_type_model->find_all(),
                'data' => []
            ];
            ...
            if ($response['find']) {
                ...
                {
                    $options = [
                        'edit' => '<a title="Editar Registro" href="' . base_url('materia_prima/cadastro/tipo_moldado/frm/' . $response['find'][$i]['id']) . '" class="btn btn-sm btn-dark"><i class="fas fa-pencil-alt fa-fw"></i> Editar</a>',
                    ...
                    $response['find'][$i]['package_quantity'] = number_format($response['find'][$i]['package_quantity'], 2, ',', '.');
        ...
        $this->load->view('raw_material/molded_type/list', $data);
    ...
    public function form($id = null)
    {
        if ($this->input->is_ajax_request()) {
            ...
            $this->form_validation->set_rules('package_quantity', 'quantidade peças por pacote', 'required');
        ...
        if ($id && !$this->molded_type_model->find_by_id($id)) {
            $this->session->set_flashdata('notification', CONF_MESSAGE_UPDATE_INVALID);

            redirect('materia_prima/cadastro/tipo_moldado/lst');
        }

        if ($id) {
            $molded_type = $this->molded_type_model->find_by_id($id);
        } else {
            $molded_type = [
                'id' => '',
                'description' => '',
        ...
        $data = [
            ...
            'frame_time' => '',
            'next_link' => '',
            'molded_type' => $molded_type
        ];

        $this->load->view('template/header_html');
        $this->load->view('template/header', $header);
        $this->load->view('raw_material/molded_type/form', $data);
        $this->load->view('template/footer');
        $this->load->view('template/footer_html');
    }
    ...
    {
        if ($this->molded_type_model->save()) {
            $response = [
    ...
    {
        if ($this->molded_type_model->save($id)) {
            $this->session->set_flashdata('notification', CONF_MESSAGE_UPDATE_SUCCESS);

            $response = [
                'redirect' => base_url('materia_prima/cadastro/tipo_moldado/lst')
            ];
    ...
    public function delete()
    {
        if ($this->input->is_ajax_request()) {
            ...
            if (!$this->molded_type_model->find_by_id($this->input->post('id'))) {
                $response = [
            ...
            if ($this->molded_type_model->destroy($this->input->post('id'))) {
