...
class Operator extends CI_Controller
{
    ...
    public function index()
    {
        if ($this->input->is_ajax_request()) {
            $response = [
                'find' => $this->operator_model->find_all(),
                'data' => []
            ];
            ...
            if ($response['find']) {
                ...
                {
                    $options = [
                        'edit' => '<a title="Editar Registro" href="' . base_url('materia_prima/cadastro/operador/frm/' . $response['find'][$i]['id']) . '" class="btn btn-sm btn-dark"><i class="fas fa-pencil-alt fa-fw"></i> Editar</a>',
        ...
        $this->load->view('raw_material/operator/list', $data);
    ...
    {
        if ($this->input->is_ajax_request()) {
            $this->form_validation->set_rules('sector', 'setor', 'required');
        ...
        if ($id && !$this->operator_model->find_by_id($id)) {
            $this->session->set_flashdata('notification', CONF_MESSAGE_UPDATE_INVALID);

            redirect('materia_prima/cadastro/operador/lst');
        }

        $this->load->model('raw_material/sector_model', 'sector_model');
        $sectors = $this->sector_model->find_all();

        if ($id) {
            $operator = $this->operator_model->find_by_id($id);
        } else {
            $operator = [
                'id' => '',
                'sector' => '',
        ...
        $this->load->view('template/header_html');
        $this->load->view('template/header', $header);
        $this->load->view('raw_material/operator/form', $data);
        $this->load->view('template/footer');
        $this->load->view('template/footer_html');
    }
    ...
    {
        if ($this->operator_model->save($id)) {
            $this->session->set_flashdata('notification', CONF_MESSAGE_UPDATE_SUCCESS);

            $response = [
                'redirect' => base_url('materia_prima/cadastro/operador/lst')
            ];
    ...
    public function delete()
    {
        if ($this->input->is_ajax_request()) {
            ...
            if (!$this->operator_model->find_by_id($this->input->post('id'))) {
                $response = [
            ...
            if ($this->operator_model->destroy($this->input->post('id'))) {
