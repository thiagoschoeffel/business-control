...
class Machine extends CI_Controller
{
    ...
    public function index()
    {
        if ($this->input->is_ajax_request()) {
            $response = [
                'find' => $this->machine_model->find_all(),
                'data' => []
            ];
            ...
            if ($response['find']) {
                ...
                {
                    $options = [
                        'edit' => '<a title="Editar Registro" href="' . base_url('materia_prima/cadastro/maquina/frm/' . $response['find'][$i]['id']) . '" class="btn btn-sm btn-dark"><i class="fas fa-pencil-alt fa-fw"></i> Editar</a>',
        ...
        $this->load->view('raw_material/machine/list', $data);
    ...
    public function form($id = null)
    {
        ...
        if ($id && !$this->machine_model->find_by_id($id)) {
            $this->session->set_flashdata('notification', CONF_MESSAGE_UPDATE_INVALID);

            redirect('materia_prima/cadastro/maquina/lst');
        }
        ...
        if ($id) {
            $machine = $this->machine_model->find_by_id($id);
        } else {
            $machine = [
                'id' => '',
                'sector' => '',
        ...
        $this->load->view('template/header_html');
        $this->load->view('template/header', $header);
        $this->load->view('raw_material/machine/form', $data);
        $this->load->view('template/footer');
        $this->load->view('template/footer_html');
    }
    ...
    {
        if ($this->machine_model->save($id)) {
            $this->session->set_flashdata('notification', CONF_MESSAGE_UPDATE_SUCCESS);

            $response = [
                'redirect' => base_url('materia_prima/cadastro/maquina/lst')
            ];
    ...
    public function delete()
    {
        if ($this->input->is_ajax_request()) {
            ...
            if (!$this->machine_model->find_by_id($this->input->post('id'))) {
                $response = [
